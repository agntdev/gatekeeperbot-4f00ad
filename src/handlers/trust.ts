import { Composer } from "grammy";
import type { Ctx } from "../bot.js";
import { inlineButton, inlineKeyboard } from "../toolkit/index.js";
import { now } from "../types.js";
import { getMember, setMember, addModerationAction, getGroupSettings, setGroupSettings } from "../data.js";

const composer = new Composer<Ctx>();

composer.command("trust", async (ctx) => {
  const chatId = String(ctx.chat?.id ?? "");
  const actorId = String(ctx.from?.id ?? "");
  if (!chatId || !actorId || !ctx.message) return;

  const args = ctx.message.text.split(/\s+/).slice(1);
  if (args.length < 1) {
    await ctx.reply("Usage: /trust @username", {
      reply_markup: inlineKeyboard([[inlineButton("⬅️ Back to menu", "menu:main")]]),
    });
    return;
  }

  const targetUsername = args[0].replace("@", "");

  const member = await getMember(chatId, targetUsername);
  if (!member) {
    await ctx.reply(`Couldn't find @${targetUsername} in this group.`, {
      reply_markup: inlineKeyboard([[inlineButton("⬅️ Back to menu", "menu:main")]]),
    });
    return;
  }

  member.trusted = true;
  member.verified = true;
  await setMember(member);

  const settings = await getGroupSettings(chatId);
  if (!settings.trustedUsers.includes(targetUsername)) {
    settings.trustedUsers.push(targetUsername);
    await setGroupSettings(settings);
  }

  await addModerationAction({
    id: `${chatId}:${now()}`,
    chatId,
    type: "trust",
    targetUserId: member.userId,
    targetUsername,
    actorUserId: actorId,
    actorUsername: ctx.from?.username,
    reason: "Marked as trusted",
    timestamp: now(),
  });

  await ctx.reply(`✅ @${targetUsername} is now trusted. They're exempt from auto-actions.`, {
    reply_markup: inlineKeyboard([[inlineButton("⬅️ Back to menu", "menu:main")]]),
  });
});

export default composer;
